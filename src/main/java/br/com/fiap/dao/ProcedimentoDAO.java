...
public class ProcedimentoDAO {
    public void inserir(Procedimento p) throws Exception {
        String sql = "INSERT INTO T_TDB_PROCEDIMENTO (ID_PROCEDIMENTO, ID_ATENDIMENTO, NM_PROCEDIMENTO, DURACAO) VALUES (?, ?, ?, ?)";
        try (Connection conn = new ConexaoFactory().conexao(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            ps.setString(3, p.getNmProcedimento());
            ps.setString(4, p.getDuracao());
            ps.executeUpdate();
            conn.commit();
        }
    }
