...
public class TriagemDAO {
    public void inserir(Triagem t) throws Exception {
        String sql = "INSERT INTO T_TDB_TRIAGEM (ID_TRIAGEM, DT_HR, ID_DENTISTA, ID_BENEFICIARIO, DS_PRIORIDADE, DS_LOCAL) VALUES (SQ_TRIAGEM.NEXTVAL, ?, ?, ?, ?, ?)";
    ...
    public List<Triagem> listar() throws Exception {
        ...
        try (Connection conn = new ConexaoFactory().conexao(); PreparedStatement ps = conn.prepareStatement("SELECT * FROM T_TDB_TRIAGEM"); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                lista.add(new Triagem(rs.getString("ID_TRIAGEM"), rs.getTimestamp("DT_HR").toLocalDateTime(), rs.getString("ID_DENTISTA"), rs.getString("ID_BENEFICIARIO"), rs.getString("DS_PRIORIDADE"), rs.getString("DS_LOCAL")));
            }
        }
        return lista;
    }
